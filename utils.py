...
def normalize_adj_tensor(mx):
     rowsum =torch.sparse.sum(mx, dim=1).to_dense()
    #  print(rowsum)
    #  exit()
     r_inv_sqrt = torch.pow(rowsum, -0.5)
    #  r_inv_sqrt[r_inv_sqrt == float('inf')] = 0.
    #  r_mat_inv_sqrt = torch.sparse.diag(r_inv_sqrt.flatten())
     r_mat_inv_sqrt = torch.diag(r_inv_sqrt).to_sparse_coo()
    #  print(r_mat_inv_sqrt)
    #  exit()
    #  r_mat_inv_sqrt = torch.sparse.FloatTensor(torch.arange(mx.size(0)), r_inv_sqrt)
    # 进行归一化操作
     normalized_mx = r_mat_inv_sqrt @ mx @ r_mat_inv_sqrt
...
def action_matrix(adj,num_nodes):
    full_1 = torch.ones(num_nodes,num_nodes)
    adjcomplement = full_1 - adj
    # C = adjcomplement-adj

    return adjcomplement.to_sparse_coo()

def relax_bernoulli(logits,temperature):
...
def process_pygdata(pygdata):
    adj = pygutiles.add_self_loops(pygdata)
    adj = pygutiles.to_torch_coo_tensor(adj)
    adj = normalize_adj_tensor(adj)       
    return adj

def get_A_aug(A,num_nodes,M):
        adjcomplement = action_matrix(A,num_nodes)
        M_edge = M*A
        M_noedge =M*adjcomplement 
        anction_edge = RelaxedBernoulli(temperature=0.01,probs=torch.sub(1, M_edge.to_dense())).sample().to_sparse_coo()
        anction_noedge = RelaxedBernoulli(temperature=0.01,probs=M_noedge.to_dense()).sample().to_sparse_coo()
        # anction_edge = RelaxedBernoulli(temperature=0.01,probs=torch.sub(1, M_edge)).sample()
        # anction_noedge = RelaxedBernoulli(temperature=0.01,probs=M_noedge).sample()
        A = A -A*anction_edge+adjcomplement*anction_noedge
        # print('check 2')
        # exit()
        normaliz_dad_aug = normalize_adj_tensor(A)
        return normaliz_dad_aug,A