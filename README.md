...
Construct feature graphs for the dataset for subsequent training. After the feature graph is constructed, it will be loaded automatically in subsequent training
```
python constructfeaturegraph.py

```
## Run
All datasets can be found in DGL or pyg open source framework for automatic download,  and save to  dataset folder. Here we take Cora dataset as an example to show how to run
```
